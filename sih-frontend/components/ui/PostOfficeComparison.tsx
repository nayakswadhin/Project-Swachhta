...
interface PostOfficeComparisonProps {
  postOffices: PostOffice[];
  loading: boolean;
}

const PostOfficeComparison: React.FC<PostOfficeComparisonProps> = ({
  postOffices,
  loading,
}) => {
  ...
  const office1 = postOffices.find(
    (office) => office.data.name === selectedOffice1
  )?.data;
  const office2 = postOffices.find(
  ...
  const ScoreCard = ({ office, isLeft = true }: { office: PostOfficeData | undefined; isLeft?: boolean }) => {
    if (!office) return (
          ...
          {metrics.map((metric) => (
            <div key={metric.key} className="flex items-center justify-between">
              ...
              <span className="text-lg font-semibold text-emerald-600">
                {formatScore(office[metric.key as keyof PostOfficeData] as number)}
              </span>
            </div>
          ))}
        </div>
    ...
    <div className="max-w-6xl mx-auto p-6">
      <div className="bg-gradient-to-br from-emerald-50 to-white p-8 rounded-2xl shadow-lg">
        <div className="flex items-center gap-3 mb-8">
          ...
          <h2 className="text-3xl font-bold text-gray-800">Post Office Comparison</h2>
        </div>
            ...
            <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 space-y-[2.35rem] bg-white px-4 py-2 rounded-lg shadow-md border border-emerald-100">
              {metrics.map((metric) => (
                <div key={metric.key} className="flex items-center justify-center">
                  {getComparisonIcon(
                    office1[metric.key as keyof PostOfficeData] as number,
                    office2[metric.key as keyof PostOfficeData] as number
                  )}
                </div>
              ))}
            </div>
...
export default PostOfficeComparison;